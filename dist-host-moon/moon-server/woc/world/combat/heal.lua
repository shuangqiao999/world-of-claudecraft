...
function M.healingThreat(source, target, healed, entities, threatMod)
    ...
    for mid, m in pairs(entities) do
        if m.kind == "mob" and not m.dead and m.hostile and m.inCombat and m.threat then
            -- 目标自身或其宠物在 mob 仇恨表上 (TS threatEntryMatchesEntity)
            local matches = m.threat[target.id]
            if not matches and target.kind == "player" then
                for tid, _ in pairs(m.threat) do
