...
local DUAL_WIELD_WHITE_MISS_PENALTY = 0.19
...
-- swingMissChance: 方向性未命中 (玩家对 mob 应用 hitBonus, mob 对玩家有上限)
local function swingMissChance(attacker, defender)
    local base = 0.05 + math.max(0, (defender.level or 1) - (attacker.level or 1)) * 0.005
    ...
    if mobToPlayer and playerSide then
        base = math.min(base, 0.2)  -- MOB_VS_PLAYER_MAX_MISS
    end
...
function M.calcPhysical(attacker, defender, opts)
    ...
    local apDmg = (ap / 14) * apSwingSpeed * 0.5
    ...
    if hitResult.result == "block" then
        rawDmg = math.max(0, rawDmg - getBlockValue(defender))
    end
