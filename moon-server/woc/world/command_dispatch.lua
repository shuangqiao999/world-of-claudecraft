...
function H.market_list(ctx, pid, cmd)
    ...
    -- 上架时从背包移除 (进入拍卖行 escrow)
    meta.inventory[slot] = nil
    ctx.marketOp(pid, { op = "list_item", pid = pid, item = held, price = n(cmd.price) or 0 })
    return true
end
...
function H.market_buy(ctx, pid, cmd)
    local meta = ctx.players[pid]
    if not meta then return false end
    ctx.marketOp(pid, { op = "buy", pid = pid, listingId = n(cmd.id) }, function(data, err)
        if data and data.price then
            -- 经济结算: 买家扣铜币 + 得物品
            if (meta.copper or 0) < data.price then
                ctx.noteEvents({ { type = "log", text = "Not enough copper", pid = pid } })
                return
            end
            meta.copper = (meta.copper or 0) - data.price
            if data.item_data and type(data.item_data) == "table" then
                ctx.inventory.addItem(meta, data.item_data)
            end
            ctx.noteEvents({ { type = "log", text = "Bought item for " .. tostring(data.price) .. " copper", pid = pid } })
        end
    end)
    return true
end
...
function H.market_collect(ctx, pid, cmd)
    local meta = ctx.players[pid]
    if not meta then return false end
    ctx.marketOp(pid, { op = "collect", pid = pid }, function(data, err)
        if data and type(data) == "table" and #data > 0 then
            local total = 0
            local ids = {}
            for _, row in ipairs(data) do
                total = total + (row.price or 0)
                table.insert(ids, row.id)
            end
            meta.copper = (meta.copper or 0) + total
            ctx.marketOp(pid, { op = "mark_collected", pid = pid, ids = ids })
            ctx.noteEvents({ { type = "log", text = "Collected " .. tostring(total) .. " copper", pid = pid } })
        end
    end)
    return true
end
...
function H.mail_send(ctx, pid, cmd)
    ...
    if copper > (meta.copper or 0) then
        ctx.noteEvents({ { type = "log", text = "Not enough copper", pid = pid } })
        return false
    end
    -- 收集多个附件 (全部物品数据 + 槽位)
