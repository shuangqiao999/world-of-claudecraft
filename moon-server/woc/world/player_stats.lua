...
-- 耐力 → HP 公式: Classic-era rules: first 20 stamina gives 1 hp each, the rest 10 hp each.
local function hpFromStamina(sta)
    local s = math.max(0, sta)
    return math.min(s, 20) + math.max(0, s - 20) * 10
end

-- 智力 → Mana 公式: first 20 intellect gives 1 mana each, the rest 15 mana each.
local function manaFromIntellect(intellect)
    local i = math.max(0, intellect)
