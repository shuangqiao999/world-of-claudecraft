...
function M._contributors(mob, players)
    local seen = {}
    local out = {}
    for attackerId, _ in pairs(threatMod.getThreats(mob.id)) do
...
function M.tick(entities, players, createMobFn, gridModule, currentTime)
    ...
    if liveId then
        ...
        elseif not boss.dead then
            ...
            -- 记录永久伤害者 (bossDamagers 来自全局仇恨表)
            for attackerId, _ in pairs(threatMod.getThreats(boss.id)) do
        ...
        if boss and boss.dead then
            ...
            -- 结算个人拾取: 权威来源是存活期累积的 bossDamagers (每 tick 从仇恨表同步)。
            -- 威胁表在 Boss 死亡时已被常规死亡路径 mobAI.cleanup 清空, 故此处 _contributors 恒空,
            -- 仅作兜底合并 (TS 原设计为死亡时在清仇恨前结算, 这里改为依赖存活期累积)。
            ...
            if boss.corpseTimer <= 0 then
                require("world.mob.ai").cleanup(liveId)
                gridModule.remove(boss)
                entities[liveId] = nil
                worldBossState.entityIds[1] = nil
                worldBossState.currentDef = nil
