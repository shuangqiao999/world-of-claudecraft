...
--- 内部物品 → 客户端 wire InvSlot 形状
--- Moon 内部 { id, name, type, ... } → 客户端 { itemId, count }
function M.toWireItem(item)
    if not item then return nil end
    return {
        itemId = item.id or item.itemId,
        count = item.count or 1,
    }
end

--- 0 基 Lua 背包表 → 密集 1 基 wire 数组 (json.encode 输出为 JSON 数组 `[]`)
--- 跳过空槽 (与客户端 InvSlot[] 密集数组模型一致)
function M.toWireArray(inv, size)
    size = size or 20
    local arr = {}
    if not inv then return arr end
    for i = 0, size - 1 do
        local item = inv[i]
        if item then
            arr[#arr + 1] = M.toWireItem(item)
        end
    end
    return arr
end

--- 装备表 (slot → 完整物品) → 客户端 (slot → itemId 字符串)
--- 客户端 equipment: Partial<Record<EquipSlot, string>>
function M.equipmentToWire(equipment)
    local wire = {}
    if not equipment then return wire end
    for slot, item in pairs(equipment) do
        if item and (item.id or item.itemId) then
            wire[slot] = item.id or item.itemId
        end
    end
    return wire
end
