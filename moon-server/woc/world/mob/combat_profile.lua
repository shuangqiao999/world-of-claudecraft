...
local builtCache = {}      -- templateId → 从模板构建的 profile (共享, 避免每 mob 重建)
local scaledCache = {}     -- "templateId:level" → 缩放后的 profile (共享, 避免每 mob 深拷贝)

function M.getProfile(templateId)
    local p = PROFILES[templateId]
    if p then return p end
    local cached = builtCache[templateId]
    if cached then return cached end
    -- 从 proto/mobs.json 构建战斗轮廓 (TS MobCombatProfile 等效)
    local tpl = M._template(templateId)
    if tpl then
        local built = M.buildProfileFromTemplate(tpl)
        builtCache[templateId] = built
        return built
    end
    return PROFILES["default"]
end

--- 获取按等级缩放后的共享 profile (缓存, 避免每个 mob 深拷贝一份独立表)
function M.getScaledProfile(templateId, level)
    local lvl = level or 1
    local key = templateId .. ":" .. lvl
    local cached = scaledCache[key]
    if cached then return cached end
    local scaled = M.scaleProfile(M.getProfile(templateId), lvl)
