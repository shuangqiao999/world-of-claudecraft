...
--- 降职 (降低等级: member→member, officer→member, 不可降 leader)
function M.demote(charId, targetId)
    local guild, err = M._requireOfficer(charId)
    if not guild then return false, err end
    if charId == targetId then return false, "Cannot demote yourself" end
    local targetGuild = dbCall("getGuildByCharacter", targetId)
    if not targetGuild or targetGuild.id ~= guild.id then return false, "Target not in your guild" end
    if (targetGuild.rank or RANK.MEMBER) <= RANK.LEADER then return false, "Cannot demote the leader" end
    local newRank = math.min(RANK.MEMBER, (targetGuild.rank or RANK.MEMBER) + 1)
    local _, err2 = dbCall("setGuildRank", guild.id, targetId, newRank)
    if err2 then return false, err2 end
    return true, "Rank updated"
end

--- 转让会长 (当前 leader → 目标 member, 自己降为 officer)
function M.transfer(charId, targetId)
    local guild, err = dbCall("getGuildByCharacter", charId)
    if not guild then return false, "Not in a guild" end
    if (guild.rank or RANK.MEMBER) ~= RANK.LEADER then return false, "Only the leader can transfer" end
    local targetGuild = dbCall("getGuildByCharacter", targetId)
    if not targetGuild or targetGuild.id ~= guild.id then return false, "Target not in your guild" end
    -- 目标升为 leader, 自己降为 officer
    dbCall("setGuildRank", guild.id, targetId, RANK.LEADER)
    dbCall("setGuildRank", guild.id, charId, RANK.OFFICER)
    return true, "Leadership transferred"
end

--- 设置公会公告
function M.setMotd(charId, motd)
    local guild, err = M._requireOfficer(charId)
    if not guild then return false, err end
    dbCall("setGuildMotd", guild.id, motd)
...
moon.dispatch("lua", function(sender, session, msg)
    ...
    elseif op == "guild_promote" then
        ...
        resp = ok and { ok = true, data = data } or { ok = false, error = data }
    elseif op == "guild_demote" then
        local ok, data = M.demote(charId, msg.targetId)
        resp = ok and { ok = true, data = data } or { ok = false, error = data }
    elseif op == "guild_transfer" then
        local ok, data = M.transfer(charId, msg.targetId)
        resp = ok and { ok = true, data = data } or { ok = false, error = data }
    elseif op == "guild_set_motd" then
        local ok, data = M.setMotd(charId, msg.motd)
        resp = ok and { ok = true, data = data } or { ok = false, error = data }
