...
`snap.self` 新增 `cst`（字符串，取值见上表）。客户端应据此渲染逃跑 / 战斗状态（未知字段可忽略，不破坏现有解析）。
...
4. **异常兜底**：目标死亡/消失/迁移 → 回 `idle`；断线重连清自动战斗状态；死亡 → `dead`。

## 待办（后续 Phase）

- 跨分片 ghost 目标的 `pvp_attack` 同意转发与 `combatState` 同步。
- 通缉/恶名系统（攻击平民 NPC → 守卫敌视）。
